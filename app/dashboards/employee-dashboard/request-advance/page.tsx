...
const MetricCard = ({
  ...
  <div className="relative bg-white/50 dark:bg-white/4 backdrop-blur-xl rounded-2xl p-5 border border-white/60 dark:border-white/10 overflow-hidden group transition-all duration-300 hover:border-white/80 dark:hover:border-white/20">
    ...
    <div className="relative z-10">
      ...
      <p className="text-xl font-bold text-slate-900 dark:text-white tracking-tight tabular-nums">{value}</p>
      <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mt-1">{label}</p>
      {subtext && <p className="text-[10px] text-slate-400/60 mt-0.5">{subtext}</p>}
    </div>
  </div>
);
...
const CircularAmountSelector = ({ value, max, currency = 'KES' }: { value: number; max: number; currency?: string }) => {
  ...
  return (
    <div className="relative w-56 h-56 mx-auto">
      {/* Ambient */}
      <div className="absolute inset-4 rounded-full blur-2xl opacity-10" style={{ background: 'radial-gradient(circle, #10b981, transparent)' }} />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <p className="text-[9px] font-bold uppercase tracking-[0.25em] text-slate-400 mb-1">Requesting</p>
        ...
        <div className="mt-2 px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400"
...
export default function RequestAdvance() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (!res.ok) { toast.error('Failed to load portal data'); return; }
  ...
  const isVerified = employee?.status === 'approved' || employee?.status === 'active';
  ...
  if (loading) return (
    <EmployeePortalLayout title="Withdrawal">
      <div className="flex flex-col items-center justify-center py-40 gap-4">
        ...
        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Verifying limit…</p>
      </div>
    ...
    <EmployeePortalLayout title="Access Restricted">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="grid sm:grid-cols-3 gap-4">
          <MetricCard icon={Shield} label="Account Status" value={isPending ? 'Pending' : 'Incomplete'} accent="#f59e0b" />
          <MetricCard icon={Wallet} label="Advance Limit" value={formatCurrency(0, currency)} accent="#3b82f6" />
          <MetricCard icon={Clock} label="Review Time" value="24–48h" subtext="Estimated duration" accent="#8b5cf6" />
        </div>
        ...
        <div className="bg-white/50 dark:bg-white/3 backdrop-blur-xl rounded-3xl border border-white/60 dark:border-white/10 p-10 text-center space-y-6">
          ...
          <div className="max-w-md mx-auto space-y-2">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">
              {isPending ? 'Application Under Review' : 'KYC Verification Required'}
            </h2>
    ...
    <EmployeePortalLayout title="Wage Advance">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <MetricCard icon={TrendingUp} label="Available Wages" value={formatCurrency(employee?.earned_wages || 0, currency)} subtext="Accrued this month" accent="#3b82f6" />
          <MetricCard icon={Shield} label="Maximum Limit" value={formatCurrency(employee?.advance_limit || 0, currency)} subtext="Based on risk score" accent="#10b981" />
          <MetricCard icon={Wallet} label="Net Disbursement" value={formatCurrency(netAmount, currency)} subtext={`After ${feePercentage}% fee`} accent="#10b981" />
        </div>
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <Label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Adjust amount</Label>
                    <span className="text-sm font-bold text-emerald-500">{formatCurrency(amount, currency)}</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[...quickAmounts.filter(a => a < maxAmount), maxAmount].slice(-4).map((amt) => (
                    <button
                      key={amt}
                    ...
                    >
                      {amt === maxAmount ? 'Max' : formatCurrency(amt, currency).split('.')[0].replace(getCurrencySymbol(currency), '').trim()}
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white/50 dark:bg-white/3 backdrop-blur-xl rounded-3xl border border-white/60 dark:border-white/10 p-6 space-y-4">
              ...
              <div className="space-y-2">
                {[
                  { label: 'Requested', value: formatCurrency(amount, currency), color: 'text-slate-900 dark:text-white' },
                  { label: `Fee (${feePercentage}%)`, value: `−${formatCurrency(feeAmount, currency)}`, color: 'text-red-500' },
                ].map((row) => (
                  <div key={row.label} className="flex justify-between items-center py-2.5 px-3 rounded-xl bg-slate-50 dark:bg-white/3 border border-slate-100 dark:border-white/5">
                    <span className="text-xs text-slate-400 font-semibold">{row.label}</span>
                    <span className={cn("font-bold text-sm tabular-nums", row.color)}>{row.value}</span>
                  </div>
                ))}
              </div>

              <div className="pt-3 border-t border-slate-100 dark:border-white/10 flex justify-between items-center">
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Total Payout</span>
            ...
            <div className="relative bg-slate-900 dark:bg-white/4 backdrop-blur-xl rounded-3xl border border-white/10 p-6 overflow-hidden">
              ...
              <div className="relative z-10 space-y-4">
                <p className="text-[9px] font-bold uppercase tracking-widest text-white/40">Disbursement Method</p>
                <div>
              ...
              {showMethodSelector && (
                <div className="absolute inset-0 bg-white dark:bg-slate-900 z-50 rounded-3xl p-5 animate-in fade-in zoom-in-95 duration-200">
                  <div className="flex justify-between items-center mb-4">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Select Method</p>
                    <button onClick={() => setShowMethodSelector(false)} className="text-slate-400 hover:text-slate-700 dark:hover:text-white transition-colors">
                    ...
                    {[
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">{m.sub}</p>
                        </div>
                        {disbursementMethod === m.id && <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />}
                      </button>
                    ))}
                  </div>
                </div>
            ...
            >
              {submitting
                ...
                : <span className="flex items-center gap-2"><Zap className="w-4 h-4 fill-current" /> Request Advance</span>}
            </Button>
          </div>
        </div>
      </div>
