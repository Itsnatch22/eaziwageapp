...
const MainStatsCard = ({
  ...
  const capacityPct =
    declaredCount > 0
      ? Math.min(Math.round((total / declaredCount) * 100), 100)
      : 0;
  ...
  const status = statusConfig[employer?.status || ""] || {
    label: employer?.status || "Not Started",
      ...
      <div className="relative w-40 h-40 mx-auto mb-6">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-3xl font-bold text-slate-900 dark:text-white">
            {declaredCount > 0 ? (
              <AnimatedCounter value={total} suffix={`/${declaredCount}`} />
            ) : (
      ...
      {trend.map((t, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
          <div
            ...
            style={{
              height: `${Math.max((t.amount / max) * 40, t.amount > 0 ? 3 : 1)}px`,
            }}
...
const PayrollHealthCard = ({
  lastSync,
}: {
  lastSync: PeriodData["last_sync"];
}) => {
  const statusConfig = {
    success: {
      label: "Healthy",
  ...
  return (
    <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/50 dark:border-slate-700/30 flex flex-col h-full">
      ...
      <Link
        ...
        className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 group"
      >
  ...
  return (
    <div className="bg-linear-to-br from-indigo-500/10 to-purple-500/10 dark:from-indigo-500/20 dark:to-purple-500/20 backdrop-blur-sm rounded-2xl p-6 border border-indigo-500/20 shadow-lg shadow-indigo-500/5">
      ...
      <p className="text-sm text-slate-600 dark:text-slate-400 mb-5 leading-relaxed">
        Share this unique code with your employees. They&apos;ll use it during
        registration to link their accounts to your company.
      </p>
      ...
      <div className="mt-5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50">
        <p className="text-[11px] text-slate-500 dark:text-slate-400 italic text-center leading-tight">
          &quot;Share this code to get your team started on EaziWage
          today!&quot;
        </p>
      </div>
    </div>
...
export default function EmployerDashboard() {
    ...
    async (options?: { silent?: boolean }) => {
      ...
      try {
        ...
        const incomplete =
          ...
          String(profile.status).toLowerCase() === "draft";
        if (incomplete) {
  ...
  useEffect(() => {
    ...
    const employerChannel = supabase
      ...
      .on(
        "postgres_changes" as const,
        {
          ...
          schema: "public",
          table: "employee_onboarding",
          filter: `employer_id=eq.${data.employer.id}`,
        },
        () => {
          handleUpdate();
        },
    ...
    return () => {
      ...
      (supabase as unknown as SupabaseWithChannel).removeChannel(
        employerChannel,
      );
    };
  }, [user?.id, data.employer?.id, load]);

  const disbursedTrend = computeTrend(
    data.curr?.advances.total_amount ?? 0,
    data.prev?.advances.total_amount ?? 0,
  );
  const feesTrend = computeTrend(
    data.curr?.advances.total_fees ?? 0,
    data.prev?.advances.total_fees ?? 0,
  );
  const avgAdvanceTrend = computeTrend(
    data.curr?.advances.avg_amount ?? 0,
    data.prev?.advances.avg_amount ?? 0,
  );
  const utilizationTrend = computeTrend(
    data.curr?.employees.utilization_rate ?? 0,
    data.prev?.employees.utilization_rate ?? 0,
  );
      ...
      <EmployerPortalLayout>
        <div className="max-w-lg mx-auto py-16 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-8 max-w-md mx-auto">
            Set up your company profile to start offering EaziWage to your
            employees and unlock all features.
          </p>
  ...
  return (
    <EmployerPortalLayout employer={data.employer}>
      <UserOnboardingGuide
        role="employer"
        show={data.employer?.status === "approved"}
      />
      <div className="max-w-7xl mx-auto space-y-6">
        {data.employer?.status === "rejected" && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-[2rem] p-6 flex flex-col md:flex-row items-center justify-between gap-6 animate-in fade-in slide-in-from-top-4 duration-500 shadow-lg shadow-red-500/5">
            <div className="flex items-start gap-4 text-left">
              ...
              <div>
                <h3 className="text-lg font-bold text-red-900 dark:text-red-200 uppercase tracking-tight">
                  Application Rejected
                </h3>
                <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                  Your company verification was not approved. Reason:{" "}
                  {data.employer?.reviewer_notes ||
                    "Please review your business details and re-submit for approval."}
                </p>
              </div>
            </div>
            <Link
              href="/dashboards/employer-dashboard/onboarding"
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/50 dark:border-slate-700/30">
            ...
            <MiniStatRow
              ...
              value={data.curr?.advances.total ?? 0}
            />
            <MiniStatRow
              label="Disbursed"
              value={data.curr?.advances.disbursed ?? 0}
              accent
            />
            <MiniStatRow
              label="Pending"
              value={data.curr?.advances.pending ?? 0}
            />
            <MiniStatRow
              label="Rejected"
              value={data.curr?.advances.rejected ?? 0}
            />
            <MiniStatRow
              label="Mobile Money"
              value={data.curr?.advances.by_method.mobile_money ?? 0}
            />
            <MiniStatRow
              label="Bank Transfer"
              value={data.curr?.advances.by_method.bank_transfer ?? 0}
            />
          </div>
          ...
          <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/50 dark:border-slate-700/30">
            ...
            <MiniStatRow
              ...
              value={
                data.employeeStats?.total_employees ??
                data.curr?.employees.total ??
                0
              }
            />
            <MiniStatRow
              label="Active"
              value={
                data.employeeStats?.active_employees ??
                data.curr?.employees.active ??
                0
              }
              accent
            />
            <MiniStatRow
              label="Used advances"
              value={data.curr?.employees.with_advances ?? 0}
            />
            <MiniStatRow
              label="Utilization rate"
              value={`${data.curr?.employees.utilization_rate ?? 0}%`}
            ...
            {data.curr?.monthly_trend && data.curr.monthly_trend.length > 0 && (
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                <p className="text-xs text-slate-400 mb-2">
                  Monthly disbursements (6 mo.)
                </p>
                <Sparkline
                  trend={data.curr.monthly_trend}
                  currency={currency}
                />
              </div>
            )}
        ...
        <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/50 dark:border-slate-700/30">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricCard
              icon={Landmark}
              label="Annual Access Available"
              value={formatCurrency(
                data.credit?.company_credit_limit ?? 0,
                currency,
              )}
            ...
            <MetricCard
              icon={CreditCard}
              label="Monthly Access Available"
              value={formatCurrency(
                data.credit?.total_outstanding_credit ?? 0,
                currency,
              )}
            ...
            <MetricCard
              icon={Zap}
              label="Monthly EWA Utilized"
              value={formatCurrency(
                data.credit?.month_disbursed_amount ?? 0,
                currency,
              )}
            ...
            <MetricCard
              ...
              value={formatCurrency(
                data.credit?.remaining_monthly_limit ?? 0,
                currency,
              )}
          ...
          <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/50 dark:border-slate-700/30">
            ...
            <div className="space-y-3">
              <StatusItem
                ...
                value={
                  data.employer?.status === "approved"
                    ? "Fully Verified"
                    : "Under Review"
                }
                status={
                  data.employer?.status === "approved" ? "success" : "warning"
                }
              />
              <StatusItem
                icon={Building2}
              ...
              <StatusItem
                ...
                label="Payroll Cycle"
                value={
                  data.employer?.payroll_cycle
                    ? data.employer.payroll_cycle.charAt(0).toUpperCase() +
                      data.employer.payroll_cycle.slice(1)
                    : "Monthly"
                }
                status="default"
              />
              <StatusItem
                ...
                value={
                  data.employer?.status === "risk_review_in_progress" ||
                  data.employer?.risk_score === 0
                    ? "Risk review in progress"
                    : data.employer?.risk_rating
                      ? `Rating ${data.employer.risk_rating}`
                      : "Not Rated Yet"
                }
                status={
                  data.employer?.risk_rating === "A"
                    ? "success"
                    : data.employer?.status === "risk_review_in_progress" ||
                        data.employer?.risk_score === 0
                      ? "warning"
                      : data.employer?.risk_rating === "D"
                        ? "warning"
                        : "default"
                }
              />
          ...
          <div className="bg-linear-to-br from-primary/5 to-emerald-500/5 dark:from-primary/10 dark:to-emerald-500/10 backdrop-blur-sm rounded-2xl p-6 border border-primary/10 dark:border-primary/20">
            <div className="flex items-start justify-between mb-5">
              ...
              {(() => {
                if (
                  ...
                  return (
                    <div className="px-4 py-2 rounded-xl font-semibold text-sm bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300">
                      Review in Progress
                    </div>
                  );
                }
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1">
                ...
                <Link
                  href="/dashboards/employer-dashboard/risk-insights"
