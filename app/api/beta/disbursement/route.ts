...
import { checkRateLimit, betaDisbursementLimiter } from '@/lib/rate-limit';
import { betaDisbursementSchema } from '@/lib/validations/beta-disbursement';
import { Resend } from 'resend';
import { BetaDisbursementFeedback } from '@/lib/emails/BetaDisbursementFeedback';
...
export async function POST(req: NextRequest) {
    try {
        ...
        const rateLimitResult = await checkRateLimit(betaDisbursementLimiter, ip);
        ...
        const parsed = betaDisbursementSchema.safeParse(body);
        ...
        if (dbError) {
            console.error("Error inserting disbursement feedback into database:", dbError);
            return NextResponse.json(
        ...
        try {
            await resend.emails.send({
                ...
                to: SUPPORT_EMAIL,
                subject: `New Beta Disbursement Feedback Submission from ${name}`,
                replyTo: email,
        ...
        console.error("Error processing disbursement feedback:", error);
        return NextResponse.json(
            { error: "Internal server error. Please try again later." },
            { status: 500 }
        );
