...
export async function GET(req: NextRequest) {
  ...
  const rate = await checkRateLimit(apiLimiter, `mfa-status-employer:${ip}`);
  ...
  try {
    ...
    console.error("Get employer MFA status error:", error);
...
export async function POST(req: NextRequest) {
  const ip = getClientIp(req);
  const apiRate = await checkRateLimit(apiLimiter, `mfa-api-employer:${ip}`);
  ...
  try {
    ...
    if (action === 'enable') {
      const actionRate = await checkRateLimit(mfaActionLimiter, `mfa-action-employer:${user.id || ip}`);
      ...
      const friendlyName = typeof body?.friendlyName === 'string' ? body.friendlyName.trim() : 'EaziWage Employer Authenticator';
      ...
      if (error) {
        console.error('[MFA-Employer][enable] enroll error:', { userId: user.id, ip, error });
      ...
      if (factorsError) {
        console.error('[MFA-Employer][disable] listFactors error:', { userId: user.id, ip, error: factorsError });
      ...
      if (unregisterError) {
        console.error('[MFA-Employer][disable] unenroll error:', { userId: user.id, ip, error: unregisterError });
      ...
      try {
        ...
        console.error('[MFA-Employer][disable] audit insert failed:', auditErr);
      }
      ...
      try {
        ...
        try {
          ...
          console.error('[MFA-Employer][backup] failed to persist backup codes:', { userId: user.id, ip, error: persistErr });
        }

        return NextResponse.json({ success: true, backupCodes: codes }, { headers: actionHeaders });
      } catch (err) {
        console.error('[MFA-Employer][backup] generation error:', err);
        return NextResponse.json({ error: 'Failed to generate backup codes' }, { status: 500, headers: actionHeaders });
      }

    } else if (action === 'verify') {
      const verifyRate = await checkRateLimit(mfaVerifyLimiter, `mfa-verify-employer:${user.id || ip}`);
      ...
      const { data: factors, error: factorsError } = await supabase.auth.mfa.listFactors();
      if (factorsError) {
        console.error('[MFA-Employer][verify] listFactors error:', { userId: user.id, ip, error: factorsError });
      ...
      if (error) {
        console.error('[MFA-Employer][verify] verify error:', { userId: user.id, ip, error });
      ...
      try {
        ...
        console.error('[MFA-Employer][verify] audit insert failed:', auditErr);
      }
    ...
    console.error("Employer MFA action error:", error);
    const errorMessage = error instanceof Error ? error.message : "An unexpected error occurred.";
    return NextResponse.json(
      { error: errorMessage },
      { status: 500 }
