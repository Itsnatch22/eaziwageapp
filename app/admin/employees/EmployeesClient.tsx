...
export default function AdminEmployees({ initialEmployees, initialStats }: { initialEmployees?: Employee[]; initialStats?: EmployeeStats | null } = {}) {
  const { rates } = useExchangeRates();
  const [employees,         setEmployees]         = useState<Employee[]>(initialEmployees ?? []);
  const [serverStats,       setServerStats]       = useState<Stats | null>(initialStats ?? null);
        ...
        <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm rounded-2xl border border-slate-200/50 dark:border-slate-700/30 overflow-hidden">
          {loading ? (
            ...
            <EmptyState
              ...
              description={searchTerm || statusFilter || kycFilter
