-- repay_advance_to_admin() and repay_employer_liability_to_admin() previously
-- updated employer_wallets.total_repaid/outstanding_liability/reserved_amount
-- but never touched last_repayment_at, and never logged a wallet_transactions
-- row — so employers saw no repayment entries in their own transaction feed.
-- Wire both up inside the RPCs so every repayment call site (DusuPay webhook,
-- admin-manual repayment routes, payout-service.ts) gets it for free.
-- 'arrears_payment' was already a valid wallet_transactions.type value
-- (wallet_transactions_type_check) but had never actually been used.
...
BEGIN
  ...
  -- Log a wallet_transactions row so the employer's own transaction history
  -- reflects this repayment — previously only total_repaid/outstanding_liability
  -- were updated, with no corresponding entry in the employer-facing feed.
  IF v_wallet_id IS NOT NULL THEN
    INSERT INTO public.wallet_transactions (
      wallet_id, amount, type, status, description, reference, metadata
    )
    VALUES (
      v_wallet_id,
      p_amount,
...
BEGIN
  ...
  -- Log a wallet_transactions row so the employer's own transaction history
  -- reflects this repayment (same gap as repay_advance_to_admin — see there).
  IF v_wallet_id IS NOT NULL THEN
    INSERT INTO public.wallet_transactions (
      wallet_id, amount, type, status, description, reference, metadata
    )
    VALUES (
      ...
      'arrears_payment',
      'completed',
      'Payday recoupment liability settlement',
      'ARR-' || p_reference,
      jsonb_build_object('employer_id', p_employer_id, 'source_reference', p_reference)
    )
    ON CONFLICT (reference) DO NOTHING;
  END IF;
END;
